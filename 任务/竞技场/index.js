...
async function 执行单个账号模式(账号名称) {
  ...
  if (result.success) { 成功日志('执行完成'); process.exit(0); }
  else if (result.successCount > 0) { 警告日志(`部分成功：成功${result.successCount}次，已记录执行时间`); process.exit(0); }
  else { 错误日志(`执行失败: ${result.error || '全部失败'}`); process.exit(1); }
}
