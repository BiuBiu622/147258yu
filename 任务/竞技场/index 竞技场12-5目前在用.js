...
async function 切换阵容(目标阵容, 描述) {
  ...
  try {
    ...
    if (!阵容信息响应 || !阵容信息响应.body) {
      警告日志(`${描述}: 获取阵容信息失败，将直接尝试切换`);
    } else {
      // 解析body（可能是BON编码）
      ...
      } else if (阵容数据 && typeof 阵容数据 === 'object' && !阵容数据.presetTeamInfo) {
        // body是数字数组对象，转换为Uint8Array再解码
...
async function 执行竞技场任务(账号前缀 = '', 账号名称 = '') {
  ...
  let 原始阵容ID = null; // 保存原始阵容ID，用于任务结束后恢复
  ...
  try {
    ...
    if (hour < 8) {
      警告日志(`${prefix}当前时间未到8点，跳过竞技场任务`);
      return { success: false, successCount: 0, reason: '时间未到' };
    }
    if (hour >= 22) {
      警告日志(`${prefix}当前时间已过22点，竞技场已关闭`);
      return { success: false, successCount: 0, reason: '竞技场已关闭' };
    }
    ...
    if (lastLoginTime) {
      ...
      // 发送随机数种子到服务器（忽略超时错误，因为响应是SyncResp）
      try {
        await 发送指令('system_custom', {
          key: 'randomSeed',
          value: randomSeed
        }, `${prefix}发送随机种子`, 500);
      } catch (error) {
        // 忽略超时，SyncResp响应映射不匹配
      }
  ...
  for (let i = 0; i < 战斗次数; i++) {
    ...
    try {
      ...
      const targetInfo = 目标数据?.roleList?.[0]?.info || {};
      
      if (!targetId) {
        警告日志(`${prefix}第${i + 1}次战斗 - 获取目标失败，可能已无剩余次数`);
        break;
      }
      ...
      // 战斗（传递完整参数）
      const 战斗结果 = await 发送指令('fight_startareaarena', {
        targetId,
        rank: targetRank,
        score: targetScore,
        info: targetInfo,
  ...
  // 战斗结果总结
  if (成功次数 >= 战斗次数) {
    成功日志(`${prefix}竞技场战斗完成 ✓ 成功${成功次数}次`);
  } else if (成功次数 > 0) {
    警告日志(`${prefix}竞技场战斗部分成功 - 成功${成功次数}次`);
  } else {
    错误日志(`${prefix}竞技场战斗全部失败`);
  }
  信息日志('');
  ...
  for (let taskId = 1; taskId <= 10; taskId++) {
    await 发送指令('task_claimdailypoint', { taskId }, `${prefix}领取日活跃奖励${taskId}`, 500);
  }
  ...
  信息日志('');
    ...
    return {
      ...
      successCount: 成功次数,
      attemptCount: 战斗次数,
      reason: 成功次数 >= 战斗次数 ? '完成' : `仅成功${成功次数}次`
    };
  } catch (error) {
    ...
    if (原始阵容ID !== null && 原始阵容ID !== undefined) {
      try {
        ...
        警告日志(`${prefix} [异常处理] 恢复原始阵容失败: ${restoreError.message}`);
      }
    }
  ...
  try {
    ...
    try {
      ...
      if (角色信息响应 && 角色信息响应.body) {
        ...
        } else if (角色数据 && typeof 角色数据 === 'object' && !角色数据.role) {
          // body是数字数组对象，转换为Uint8Array再解码
          const keys = Object.keys(角色数据).map(k => parseInt(k)).sort((a, b) => a - b);
          if (keys.length > 0 && keys[0] === 0) {
            const arr = new Uint8Array(keys.length);
            keys.forEach((k, i) => arr[i] = 角色数据[k]);
            const { bon } = await import('../../工具/BON协议.js');
            角色数据 = bon.decode(arr);
          }
        }
        ...
        if (角色数据?.role) {
          ...
          if (状态数据 && 状态数据.dailyTask) {
            const 当前进度 = 状态数据.dailyTask.dailyPoint || 0;
            信息日志(`${账号前缀} 当前每日任务进度: ${当前进度}/110`);
            
            // 更新每日任务进度（只更新dailyTask，不影响其他数据）
            更新账号状态(tokenData.name, 状态数据);
          }
        }
      }
    } catch (error) {
      警告日志(`${账号前缀} 获取每日任务进度失败: ${error.message}`);
      // 不影响竞技场状态更新，继续执行
    }
    ...
    if (任务结果?.success) {
      成功日志(`${账号前缀} 任务执行完毕 ✓ 成功${任务结果.successCount}次 (耗时: ${执行时长}秒)`);
    } else {
      警告日志(`${账号前缀} 任务执行完毕 ⚠ 成功${任务结果?.successCount || 0}次 (耗时: ${执行时长}秒)`);
    }
    信息日志('');
    
    // 更新竞技场状态（只更新arena字段，不影响其他数据）
    更新竞技场状态(tokenData.name, {
      successCount: 任务结果?.successCount || 0,
      attemptCount: 任务结果?.attemptCount || 0,
      status: 任务结果?.success ? 'success' : (任务结果?.successCount > 0 ? 'partial' : 'failed')
    });
...
async function 执行单个账号模式(账号名称) {
  ...
  // ✅ 方案1：部分成功也记录执行时间（避免循环）
  // 如果成功次数 > 0，即使未达到全部成功，也返回退出码0（让调度器记录时间）
  // 只有全部失败（成功次数 = 0）才返回退出码1
  if (result.success) {
    成功日志('执行完成');
    process.exit(0);
  } else if (result.successCount > 0) {
    警告日志(`部分成功：成功${result.successCount}次，已记录执行时间（避免循环）`);
    process.exit(0); // 部分成功也记录时间
  } else {
    错误日志(`执行失败: ${result.error || '全部失败'}`);
    process.exit(1); // 全部失败，不记录时间，允许重试
  }
}
