...
async function 手动购买商品() {
  ...
  // 第一轮购买
  await 发送指令('store_buy', { goodsId: 1 }, '购买青铜宝箱', 250);
  await 发送指令('store_buy', { goodsId: 2 }, '购买黄金宝箱', 250);
  await 发送指令('store_buy', { goodsId: 3 }, '购买铂金宝箱', 250);
  ...
  // 刷新黑市
  信息日志('刷新黑市商品列表...');
  await 发送指令('store_refresh', { storeId: 1 }, '刷新黑市', 250);
...
// ✅ 工具函数: 发送游戏指令（使用seq/ack精准匹配，带断连检测）
async function 发送指令(cmd, body = {}, 描述 = '', 超时时间 = 3000) {
  ...
  if (!client || !client.isConnected || !client.isConnected()) {
    警告日志(`[SEQ ${seq}] 连接已断开，尝试重连...`);
    
    if (client && client.reconnect) {
      const 重连成功 = await client.reconnect();
      if (!重连成功) {
        错误日志(`[SEQ ${seq}] 重连失败，跳过: ${显示描述}`);
        return null;
      }
      // 重连成功后重新初始化消息监听器
      初始化消息监听器();
      成功日志(`[SEQ ${seq}] 重连成功，继续执行`);
    } else {
      错误日志(`[SEQ ${seq}] 连接已断开且无法重连，跳过: ${显示描述}`);
      return null;
    }
  }
  ...
  return new Promise((resolve, reject) => {
    ...
    // 发送命令（带seq）
    const 发送成功 = client.send(cmd, { ...body, seq });
    if (!发送成功) {
      clearTimeout(timeoutId);
      pendingRequests.delete(seq);
      警告日志(`[SEQ ${seq}] 发送失败: ${显示描述}`);
      resolve(null);
    }
  });
}
...
async function 执行每日任务(角色信息, 账号前缀 = '', 账号名称 = '') {
  ...
  // 钓鱼月度补齐已移至独立任务：任务/钓鱼月度补齐/index.js
...
async function 执行单个账号(tokenData, 账号索引, 总账号数) {
  ...
  try {
    ...
    // 执行每日任务（每个子任务延迟已优化，WebSocket客户端会自动保持心跳）
